...
console.log("success");
 ...
 var currentWidth = $('#mapDiv').width();
  ...
  var path = d3.geo
        ...
        var route = svg.select("path").attr("path");

        /*var route1 = svg.select("path").datum("path");
        console.log("datum is:",route1);*/
        console.log("route is:",route);
        //去掉前面两个字符
        var data = route.slice(2);
        console.log("route data is:",data);
        console.log("route's length is:",data.length);
    //按空格对字符进行分割
        res1 = data.split(" ");
    ...
    //分割后的数组进行飞行操作
        setTimeout(function(){
            for(var i=0; i<res1.length;i++){
                console.log(i);
                var startPoint = res1[i];
                console.log("startPointer",startPoint);
                if(i!=res1.length-1){
                    var endPoint = res1[i+1];
                    console.log("endPointer",endPoint);
                    fly(startPoint,endPoint);
                }else{
                    return ;
                }
            };
    ...
    function fly(origin, destination) {
        var route = svg.select("path")
            .datum({type: "LineString", coordinates: [origin, destination]})
            .attr("class", "route");
          /*  var plane = svg.selectAll("image").data([0]);
        plane.enter()
            .append("svg:image")
            .attr("xlink:href", "/img/chuan.jpg")
            .attr("width", "40")
            .attr("height", "40")
            .attr("class","boat");*/
    ...
    function transition(plane, route) {
        ...
        console.log("route node() is:",route.node());
        plane.transition()
            .duration(l * 20)
            .attrTween("transform", delta(plane, route.node()))
            .each("end", function() {console.log(111);/*route.remove()*/});
    }
    function delta(plane, path) {
        var l = path.getTotalLength();
        console.log("path.getTotalLength is ",l);
        var plane = plane;
        return function(i) {
            return function(t) {
                var p = path.getPointAtLength(t * l);
               /* console.log("p is t*l :",p)
                console.log("p.x is :", p.x);*/
